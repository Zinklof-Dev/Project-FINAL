...
{
    /// <summary>
    /// Defines a function as a command for the console to compile.
    /// </summary>
    public class Command : System.Attribute
    {
        public string callName = string.Empty;
        public bool cheat;
        public bool developmental;
    ...
    public class ShellCommandClass
    {
        ...
        public ShellCommandClass(string callName, string helpDescription, bool cheat, bool developmental, GenericCommandVariable[] genericCommandVariables, MethodInfo methodInfo)
        {
            this.callName = callName;
            this.helpDescription = helpDescription;
            this.cheat = cheat;
    ...
    public class GenericCommandVariable
    {
        ...
        public object ParseAsObject(string value)
        {
            switch (type)
            {
                ...
                default:
                    throw new ArgumentException("Bastion.ConsoleV2.GenericCommandVariable.ParseAsObject(), Arguement falls out of range exception (Unsupported value type for command)");
            }
        }
        ...
        public override string ToString()
        {
            switch (type)
            {
                ...
                case 11:
                    return "Char";
                default:
                    throw new ArgumentException("Bastion.ConsoleV2.GenericCommandVariable.ToString(), Arguement falls out of range exception (Unsupported value type for command)");
            }
        }
    }

    /// <summary>
    /// The "shell" of the console system. Behaves as the interface between the commands functions, and the user. Kind of like a shell does between a kernal and user!
    /// Basically just the guts of the command system.
    /// </summary>
    public static class Shell
    {
        ...
        [Command("The Help Command.", false, false, "Shell.Help")]
        public static void Help()
        {
            ...
            Bastion.ConsoleV2.Console.Log(result, "Help");
        }

        [Command("Enable or disable cheats.", false, false, "Shell.Cheats")]
        public static void setCheats(bool value)
        {
            Console.cheatsEnabled = value;
            Console.Log("Cheats value is now: " +  value, "Shell");
        }
        ...
        public static void CallCommand(string input)
        {
            ...
            if (command.cheat && !Console.cheatsEnabled)
            {
                Console.Log("Specified command is a cheat, but cheats are not currently enabled! (see Shell.Cheats())", "Shell");
                return;
            } 
            
            if (command.developmental && !Console.devBuild)
            {
                Console.Log("Specified command is for dev builds only. (please report, you shouldn't see this since its a final failsafe)", "Shell");
                return;
            }
